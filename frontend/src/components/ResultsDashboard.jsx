...
export default function ResultsDashboard({ analysis, session, onBack }) {
  const { linear_regression, propensity_matching, doubly_robust, double_ml, causal_forest, refutations, summary } = analysis;
  ...
  const validMethods = methods.filter(m => m.data && !m.data.error);

  const bestAte = validMethods.length > 0
    ? validMethods.reduce((a, b) => Math.abs(a.data.ate) < Math.abs(b.data.ate) ? a : b)
    : null;
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {summary.treatment} &rarr; {summary.outcome} &middot; {summary.num_rows.toLocaleString()} observations &middot; {summary.confounders.length} confounders
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 stagger-children">
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold text-gray-100">{summary.num_rows.toLocaleString()}</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-2 mb-2">
        ...
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold text-gray-100">{summary.confounders.length}</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-2xl font-bold text-gray-100">{summary.missing_values}</p>
        </div>
      </div>
                ...
                {data.error ? (
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between items-center py-1.5 border-b border-white/10">
                      ...
                      <span className="font-mono font-semibold text-gray-200">{data.original_estimate.toFixed(4)}</span>
                    </div>
                    <div className="flex justify-between items-center py-1.5 border-b border-white/10">
                      <span className="text-gray-500 text-xs">Refuted</span>
                      <span className="font-mono font-semibold text-gray-200">{data.new_estimate.toFixed(4)}</span>
                    </div>
            ...
            <tbody>
              {methods.map(({ key, label, data, color }) => (
                <tr key={key} className="border-b border-white/[0.03] hover:bg-primary-500/5 transition-colors duration-150">
                  ...
                  {data && !data.error ? (
                    <>
                      <td className="text-right py-3.5 px-4 font-mono font-semibold text-gray-100 text-sm">{data.ate.toFixed(5)}</td>
                      <td className="text-right py-3.5 px-4 font-mono text-gray-500 text-sm">
                        {data.ate_interval ? data.ate_interval[0].toFixed(5) : '\u2014'}
                      </td>
                      <td className="text-right py-3.5 px-4 font-mono text-gray-500 text-sm">
                        {data.ate_interval ? data.ate_interval[1].toFixed(5) : '\u2014'}
                      </td>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-base sm:text-lg font-semibold leading-relaxed text-white/95">
                ...
                <strong className="text-white text-xl">{bestAte.data.ate.toFixed(4)}</strong>.
                {bestAte.data.ate > 0
              ...
              <p className="text-sm text-primary-200 mt-2 leading-relaxed">
                ...
                {bestAte.data.ate_interval && (
                  <> The 95% confidence interval spans [{bestAte.data.ate_interval[0].toFixed(4)}, {bestAte.data.ate_interval[1].toFixed(4)}].</>
                )}
              </p>
            </div>
          </div>
